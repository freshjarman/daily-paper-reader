...
tags: ["query:tpp-es"]
score: 7.0
evidence: 从无标签事件数据中学习表示
tldr: 针对事件相机数据标注稀缺的问题，本文提出了SpikeCLR，一种基于脉冲神经网络（SNN）的对比自监督学习框架。通过引入空间、时间和极性变换的事件增强技术，该框架能从无标签数据中学习鲁棒的视觉表示。实验证明，在少样本和半监督场景下，该方法显著优于传统的监督学习，为低功耗嵌入式视觉应用提供了高效的预训练方案。
selection_source: fresh_fetch
motivation: 事件相机结合SNN具有高能效优势，但大规模标注数据的匮乏限制了其在复杂任务中的应用。
method: 提出SpikeCLR框架，利用代理梯度训练SNN，并设计了一套涵盖空间、时间和极性的事件专用数据增强策略。
result: 在多个主流事件数据集上，自监督预训练在少样本和半监督设置下均取得了优于监督学习的性能。
conclusion: 结合时空增强对学习事件数据的特征不变性至关重要，且学习到的表示具有良好的跨数据集迁移能力。
---

## 摘要
事件视觉传感器在高速感知方面具有显著优势，包括微秒级的时间分辨率、高动态范围和低功耗。当与脉冲神经网络（SNNs）结合时，它们可以部署在类脑硬件上，从而在嵌入式系统中实现高能效应用。然而，由于有效训练此类模型所需的大规模标记数据集稀缺，这一潜力受到了严重限制。在这项工作中，我们介绍了 SpikeCLR，这是一个对比自监督学习框架，使 SNN 能够从无标签的事件数据中学习鲁棒的视觉表示。我们利用代理梯度训练将先前的基于帧的方法适配到脉冲领域，并引入了一套利用空间、时间和极性变换的事件特定增强方法。通过在 CIFAR10-DVS、N-Caltech101、N-MNIST 和 DVS-Gesture 基准测试上的广泛实验，我们证明了在低数据量情况下，自监督预训练结合随后的微调优于监督学习，在小样本和半监督设置中取得了持续的提升。我们的消融研究表明，结合空间和时间增强对于学习事件数据中有效的时空不变性至关重要。我们进一步展示了所学习的表示可以跨数据集迁移，为在标签稀缺环境下构建强大的基于事件的模型做出了贡献。
