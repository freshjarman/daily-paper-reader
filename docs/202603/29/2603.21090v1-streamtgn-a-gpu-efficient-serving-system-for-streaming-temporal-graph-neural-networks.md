...
motivation: 现有TGN推理系统在处理新边时会触发全局节点更新，导致计算冗余且效率低下。
method: "利用时序图更新的局部性，通过脏标记传播识别受影响节点集，将计算复杂度从O(|V|)降至O(|A|)。"
result: 在多个数据集上实现了4.5倍至739倍的TGN加速，且在保持零精度损失的同时显著提升了吞吐量。
conclusion: StreamTGN是首个高效的流式TGN推理系统，通过优化内存管理和调度策略，极大地提升了动态图推理性能。
---
