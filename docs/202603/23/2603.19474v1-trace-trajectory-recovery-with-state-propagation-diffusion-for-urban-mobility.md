...
## 摘要
高质量的 GPS 轨迹对于基于位置的网络服务和智慧城市应用（包括导航、网约车和配送）至关重要。然而，由于数据采集过程中的低采样率和有限的基础设施覆盖，现实世界的轨迹通常是稀疏的，且位置点分布不均。将这些轨迹恢复为密集且连续的形式至关重要，但由于其复杂且不规则的时空模式，这具有很大的挑战性。在本文中，我们介绍了一种名为 TRACE 的新型轨迹恢复扩散模型，该模型能够从稀疏且不完整的输入中重建密集且连续的轨迹。TRACE 的核心是我们提出的状态传播扩散模型（SPDM），它集成了一种新颖的记忆机制，使得在去噪过程中，TRACE 能够保留并利用前几步的中间结果，从而有效重建那些难以恢复的轨迹段。在多个真实世界数据集上的广泛实验表明，TRACE 优于现有最先进的方法，在没有显著推理开销的情况下，准确率提升了 26% 以上。我们的工作巩固了移动和网络连接位置服务的基础，提升了数据驱动型城市应用的质量和公平性。代码可在以下网址获取：https://github.com/JinmingWang/TRACE
