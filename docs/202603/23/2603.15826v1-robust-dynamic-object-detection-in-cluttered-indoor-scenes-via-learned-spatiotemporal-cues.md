...
conclusion: 通过结合时空线索与学习到的动态先验，可以显著提高室内复杂场景下动态物体检测的可靠性。
---

## 摘要
在杂乱环境中实现可靠的动态物体检测仍然是自主导航面临的关键挑战。依赖聚类和启发式过滤的纯几何 LiDAR 流水线在动态障碍物靠近静态结构移动或仅被部分观测时，可能会漏掉这些障碍物。视觉增强方法可以提供额外的语义线索，但通常受限于闭集检测器和相机视场限制，降低了对新颖障碍物和视锥体外事件的鲁棒性。在这项工作中，我们提出了一个仅使用 LiDAR 的框架，该框架将基于时间占用网格的运动分割与学习到的鸟瞰图（BEV）动态先验相结合。融合模块在可用时优先考虑 3D 检测，同时利用学习到的动态网格来恢复那些因邻近导致的假阴性而丢失的检测。使用动作捕捉真值进行的实验表明，在高度杂乱的环境中，我们的方法比现有最先进技术实现了高出 28.67% 的召回率和 18.50% 的 F1 分数，同时保持了相当的精度和位置误差。
