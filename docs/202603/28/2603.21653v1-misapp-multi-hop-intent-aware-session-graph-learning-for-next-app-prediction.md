...
## 摘要
预测用户将启动的下一个移动应用对于主动式移动服务至关重要。然而，在现实场景中，准确预测仍具挑战性，因为用户意图在短会话内可能迅速转变，且用户特定的历史画像通常稀疏或不可用，尤其是在冷启动条件下。现有方法主要将应用使用建模为序列行为或局部会话转换，这限制了它们捕捉高阶结构依赖和演化会话意图的能力。为解决这一问题，我们提出了 MISApp，这是一个基于多跳会话图学习的无需画像的下一个应用预测框架。MISApp 构建多跳会话图以捕捉不同结构范围内的转换依赖，通过轻量级图传播学习会话表示，结合时间与空间上下文来刻画会话条件，并从最近的交互中捕捉意图演化。在两个真实应用使用数据集上的实验表明，MISApp 在标准和冷启动设置下均持续优于竞争基准模型，同时在预测准确性与实际效率之间保持了良好的平衡。进一步分析表明，学习到的跳级注意力权重与结构相关性高度一致，为所提多跳建模策略的有效性提供了可解释的证据。
