...
## 摘要
Transformer 模型重新定义了序列学习，然而点积自注意力机制为长上下文时间序列引入了二次复杂度的 Token 混合瓶颈。我们提出了相量 Transformer (Phasor Transformer) 模块，这是一种在单位圆流形 $S^1$ 上表示序列状态的原生相位替代方案。每个模块结合了轻量级的可训练相移与无参数的离散傅里叶变换 (DFT) Token 耦合，在无需显式注意力图的情况下实现了全局 $\mathcal{O}(N\log N)$ 的混合。堆叠这些模块定义了大相量模型 (LPM)。我们在合成多频基准测试的自回归时间序列预测上验证了 LPM。在极紧凑的参数预算下，LPM 学习到了稳定的全局动力学，并与传统的自注意力基准模型相比，实现了具有竞争力的预测表现。我们的结果建立了一个明确的效率-性能前沿，证明了时间序列的大模型缩放可以源于具有确定性全局耦合的几何约束相位计算，为振荡领域的扩展性时间建模提供了一条切实可行的路径。
